...
class ChatSessionController {
  ...
  async createSession(req, res) {
    try {
      ...
      res
        .status(201)
        .json({ message: "Chat session created successfully", sessionId });
    } catch (error) {
  ...
  async getSessionsForLawerChat(req, res) {
    try {
      const sessions = await ChatSessions.getSessionsForLawerChat(
        req.params.user_id
      );
      res.json(sessions);
    } catch (error) {
  ...
  async updateSession(req, res) {
    try {
      ...
      const affectedRows = await ChatSessions.updateSession(
        req.params.id,
        updateData
      );
      if (!affectedRows) {
  ...
  async getSessionsByChatbot(req, res) {
    try {
      const sessions = await ChatSessions.getSessionsByChatbot(
