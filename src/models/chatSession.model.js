...
class ChatSessions {
  ...
  static async createSession(user_id, chatbot_id) {
    const query =
      "INSERT INTO chatsessions (user_id, chatbot_id, start_time) VALUES (?, ?, NOW())";
  ...
  static async getSessionsByChatbot(chatbot_id) {
    const query =
      "SELECT * FROM chatsessions WHERE chatbot_id = ? ORDER BY start_time DESC";
    const [rows] = await db.execute(query, [chatbot_id]);
    return rows;
  }

  // lấy danh sách phiên chat cho chatbot tư vấn luật DN
  static async getSessionsForLawerChat(user_id) {
    const query =
      "SELECT * FROM chatsessions WHERE user_id = ? and chatbot_id = 86 ORDER BY start_time DESC";
  ...
  static async getSessionsByUser(user_id) {
    const query =
      "SELECT * FROM chatsessions WHERE user_id = ? ORDER BY start_time DESC";
    const [rows] = await db.execute(query, [user_id]);
    return rows;
  }
  ...
  static async deleteSession(session_id) {
    ...
    try {
      ...
      const [result] = await connection.execute(deleteSessionQuery, [
        session_id,
      ]); // Xóa phiên chat
